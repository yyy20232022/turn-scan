...
public class SpecialApi {
    ...
    public List<HistoryLowRateSlash> getHistoryLowRateSlashList(Web3j web3j, BigInteger blockNumber) throws
        ...
        if (br.isStatusOk()) {
            ...
            throw new ContractInvokeException(String.format("【查询历史低出块处罚信息出错】函数类型:%s,区块号:%s,返回数据:%s",
    ...
    private List<Node> nodeCall(Web3j web3j, BigInteger blockNumber, int funcType) throws
        ...
        if (br == null || br.getData() == null) {
            throw new BlankResponseException(String.format("【查询验证人出错】函数类型:%s,区块号:%s,返回为空!%s",
        ...
        if (br.isStatusOk()) {
            ...
            String msg = JSON.toJSONString(br);
            throw new ContractInvokeException(String.format("【查询验证人出错】函数类型:%s,区块号:%s,返回数据:%s",
                                                            funcType,
                                                            blockNumber.toString(),
                                                            msg));
        }
    ...
    public List<NodeVersion> getNodeVersionList(Web3j web3j) throws ContractInvokeException, BlankResponseException {
        ...
        CallResponse<String> br = rpc(web3j,
                                      ...
                                      InnerContractAddrEnum.NODE_CONTRACT.getAddress());
        if (br == null || br.getData() == null) {
            throw new BlankResponseException(String.format("【查询节点版本出错】函数类型:%s,返回为空!%s",
                                                           GET_NODE_VERSION,
                                                           JSON.toJSONString(Thread.currentThread().getStackTrace())));
        }
        if (br.isStatusOk()) {
            ...
            String msg = JSON.toJSONString(br);
            throw new ContractInvokeException(String.format("【查询节点版本出错】函数类型:%s,返回数据:%s",
                                                            GET_NODE_VERSION,
                                                            msg));
        }
    }
    ...
    public EpochInfo getEpochInfo(Web3j web3j, BigInteger blockNumber) throws
        ...
        CallResponse<String> br = rpc(web3j,
                                      ...
                                      InnerContractAddrEnum.NODE_CONTRACT.getAddress());
        if (br == null || br.getData() == null) {
            throw new BlankResponseException(String.format("查询历史周期信息出错【blockNumber:%s)】,返回为空!",
                                                           blockNumber));
        }
        ...
        if (br.isStatusOk()) {
            ...
            String msg = JSON.toJSONString(br);
            throw new ContractInvokeException(String.format("【查询历史周期信息出错】区块号:%s,返回数据:%s",
                                                            blockNumber,
                                                            msg));
        }
    }
    ...
    public ProposalParticipantStat getProposalParticipants(Web3j web3j, String proposalHash, String blockHash) throws
        ...
        CallResponse<String> br = rpc(web3j,
                                      ...
                                      InnerContractAddrEnum.PROPOSAL_CONTRACT.getAddress());
        if (br == null || br.getData() == null) {
            throw new BlankResponseException(String.format("查询提案参与人出错【提案Hash:%s,区块Hash:%s】",
                                                           proposalHash,
                                                           blockHash));
        }
        if (br.isStatusOk()) {
            ...
            String msg = JSON.toJSONString(br);
            throw new ContractInvokeException(String.format("【查询提案参与者出错】提案Hash:%s,区块Hash:%s,返回数据:%s",
                                                            proposalHash,
                                                            blockHash,
                                                            msg));
        }
    ...
    public List<PPosInvokeContractInput> getPPosInvokeInfo(Web3j web3j, BigInteger blockNumber) throws
        ...
        if (br.isStatusOk()) {
            ...
            String msg = JSON.toJSONString(br);
            throw new ContractInvokeException(String.format("【查询PPOS调用信息出错】函数类型:%s,区块号:%s,返回数据:%s",
    ...
    public List<AdjustParam> getStakingDelegateAdjustDataList(Web3j web3j, BigInteger blockNumber) throws
        ...
        if (br.isStatusOk()) {
            ...
            String msg = JSON.toJSONString(br);
            throw new ContractInvokeException(String.format("【查询质押委托调账信息出错】函数类型:%s,区块号:%s,返回数据:%s",
