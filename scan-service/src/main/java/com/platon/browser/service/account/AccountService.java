...
public class AccountService {
    ...
    public BigDecimal getInciteBalance(BigInteger blockNumber) {
        ...
        try {
            BigInteger balance = platOnClient.getWeb3jWrapper().getWeb3j().bubbleGetBalance(INCITE_ACCOUNT_ADDR, DefaultBlockParameter.valueOf(blockNumber)).send().getBalance();
    ...
    public BigDecimal getLockCabinBalance(BigInteger blockNumber) {
        ...
        try {
            BigInteger balance = platOnClient.getWeb3jWrapper().getWeb3j().bubbleGetBalance(RESTRICTING_ADDR, DefaultBlockParameter.valueOf(blockNumber)).send().getBalance();
            return new BigDecimal(balance);
        } catch (Exception e) {
            platOnClient.updateCurrentWeb3jWrapper();
    ...
    public BigDecimal getStakingBalance(BigInteger blockNumber) {
        ...
        try {
            BigInteger balance = platOnClient.getWeb3jWrapper().getWeb3j().bubbleGetBalance(STAKING_ADDR, DefaultBlockParameter.valueOf(blockNumber)).send().getBalance();
            return new BigDecimal(balance);
        } catch (Exception e) {
            platOnClient.updateCurrentWeb3jWrapper();
    ...
    public BigDecimal getRewardBalance(BigInteger blockNumber) {
        if (blockNumber.compareTo(BigInteger.ZERO) < 0) blockNumber = BigInteger.ZERO;
        try {
            BigInteger balance = platOnClient.getWeb3jWrapper().getWeb3j().bubbleGetBalance(REWARD_ADDR, DefaultBlockParameter.valueOf(blockNumber)).send().getBalance();
            return new BigDecimal(balance);
        } catch (Exception e) {
            platOnClient.updateCurrentWeb3jWrapper();
